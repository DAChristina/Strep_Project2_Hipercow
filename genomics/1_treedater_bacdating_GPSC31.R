...
# 
# saveRDS(treedater_GPSC2, file = "outputs/genomics/GPSC31/treedater_GPSC31.rds")
# 
# # Trial load:
# treedater_GPSC31 <- readRDS("outputs/genomics/GPSC31/treedater_GPSC31.rds")
...
run_bactdating <- function(nbIts){
  ...
  tre <- BactDating::loadGubbins("raw_data/gubbins/GPSC_31/n712_")
  ...
  dir.create("outputs/genomics/GPSC31", FALSE, TRUE)
  dir.create("pictures/genomics/GPSC31", FALSE, TRUE)
  
  res_pr <- BactDating::bactdate(tre,d,nbIts=nbIts, # Put 1e6 or 1e10 on hipercow
                                 model = "strictgamma", # "arc",
                                 showProgress = T)
  
  saveRDS(res_pr, "outputs/genomics/GPSC31/mcmc_bacdating_GPSC31.rds")
  
  # Figures!
  png("pictures/genomics/GPSC31/tree_treeCI.png", width = 24, height = 12, unit = "cm", res = 1200)
  par(mfrow = c(1,1), mar = c(3, 3, 2, 2), mgp = c(1.7, 0.7, 0), bty = "n")
  plot(res_pr,'treeCI',show.tip.label = F)
  dev.off()
  
  png("pictures/genomics/GPSC31/tree_trace1.png", width = 24, height = 12, unit = "cm", res = 1200)
  par(mfrow = c(1,1), mar = c(3, 3, 2, 2), mgp = c(1.7, 0.7, 0), bty = "n")
  plot(res_pr,'trace')
  dev.off()
  ...
  write.csv(calc_ess, "outputs/genomics/GPSC31/calc_ess.csv", row.names = TRUE)
  
  # Figures! (still failed, margin error)
  png("pictures/genomics/GPSC31/tree_trace2.png", width = 24, height = 12, unit = "cm", res = 1200)
  par(mfrow = c(1,1), mar = c(3, 3, 2, 2), mgp = c(1.7, 0.7, 0), bty = "n")
  ...
  saveRDS(rooted_tree, "outputs/genomics/GPSC31/rooted_tree.rds")
  
  png("pictures/genomics/GPSC31/tree_rootedtree.png", width = 24, height = 12, unit = "cm", res = 1200)
  par(mfrow = c(1,1), mar = c(3, 3, 2, 2), mgp = c(1.7, 0.7, 0), bty = "n")
  ...
  png("pictures/genomics/GPSC31/tree_roottotip.png", width = 24, height = 12, unit = "cm", res = 1200)
  par(mfrow = c(1,1), mar = c(3, 3, 2, 2), mgp = c(1.7, 0.7, 0), bty = "n")
  res_roottotip <- BactDating::roottotip(rooted_tree,d[,1])
  saveRDS(res_roottotip, "outputs/genomics/GPSC31/res_roottotip.rds")
  dev.off()
